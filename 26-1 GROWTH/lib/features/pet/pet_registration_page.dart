...
class _PetRegistrationPageState extends State<PetRegistrationPage> {
  ...
  Future<void> _submit() async {
    ...
    if (missingFields.isNotEmpty) {
      await _showMissingFieldsDialog(missingFields);
    ...
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => PetImageRegistrationPage(petData: payload),
      ),
    );
  ...
  List<String> _collectMissingRequiredFields() {
    ...
    if (int.tryParse(_ageController.text.trim()) == null) {
      missingFields.add('나이');
    }
    if (_breedController.text.trim().isEmpty) {
      missingFields.add('품종');
    }
    if (double.tryParse(_weightController.text.trim()) == null) {
      missingFields.add('무게');
    }

    return missingFields;
  }

  Future<void> _showMissingFieldsDialog(List<String> missingFields) {
    return showDialog<void>(
      context: context,
      builder: (context) {
        return AlertDialog(
          backgroundColor: Colors.white,
          title: const Text('필수값을 입력해 주세요'),
          content: Text('${missingFields.join(', ')} 항목이 필요합니다.'),
          actions: [
            TextButton(
            ...
            child: ListView(
              ...
              children: [
                ...
                _buildBooleanTile(
                  ...
                  onChanged: (value) =>
                      setState(() => _isDangerousBreed = value),
                ),
                const SizedBox(height: 22),
